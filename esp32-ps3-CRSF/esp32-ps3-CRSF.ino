...
int controllerChannels[19];  // Ps3 controller channels
...
void loop() {
  ...
  if (Ps3.isConnected()) {
    readController();
    rcChannels[THROTTLE] = map(controllerChannels[stick_L_Y], -128, 128, CRSF_CHANNEL_MIN, CRSF_CHANNEL_MAX);
    rcChannels[AILERON] = map(controllerChannels[stick_R_X], -128, 128, CRSF_CHANNEL_MIN, CRSF_CHANNEL_MAX);
    rcChannels[ELEVATOR] = map(controllerChannels[stick_R_Y], -128, 128, CRSF_CHANNEL_MIN, CRSF_CHANNEL_MAX);
    rcChannels[RUDDER] = map(controllerChannels[stick_L_X], -128, 128, CRSF_CHANNEL_MIN, CRSF_CHANNEL_MAX);
...
void readController() {
  ...
  // controllerChannels[stick_L_Y] = -Ps3.data.analog.stick.ly; // self-centering stick, need to ignore downwards movement
  if (Ps3.data.analog.stick.ly <= 0) {
    controllerChannels[stick_L_Y] = map(-Ps3.data.analog.stick.ly,0,128,-128,128);  // Up is negative
  } else {
    controllerChannels[stick_L_Y] = -128;  // ignore downwards movement
  }
  controllerChannels[stick_R_X] = Ps3.data.analog.stick.rx;
  controllerChannels[stick_R_Y] = -Ps3.data.analog.stick.ry;  // Up is negative
